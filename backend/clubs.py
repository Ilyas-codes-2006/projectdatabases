...
— Het MatchUp systeem
"""
        # Voeg bijlagen toe aan de e-mail
...
def review_club_request(request_id: int, action: str, mail_instance):
    ...
    In beide gevallen krijgt de aanvrager een e-mail.
    """
    from config import config_data as config
    ...
    if action == "approve":
        ...
        db.session.flush()  # krijg new_club.id

        # Voeg aanvrager toe als lid (en admin van de club)
        ...
        # Stuur bevestigingsmail naar aanvrager
        msg = Message(
...
def show_clubs():
    ...
    for c in clubs:
        ...
        # Check of gebruiker een pending join request heeft voor deze club
...
def request_join_club(club_id: int, motivation: str, mail_instance):
    """
    Stuurt een join-aanvraag naar de club admin.
    """
    from config import config_data as config
...
def review_join_request(join_request_id: int, action: str, mail_instance):
    ...
    # Check dat de huidige user club admin is van die club
