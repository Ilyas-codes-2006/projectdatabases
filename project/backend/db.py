...
def apply_match_result(match_id: int, cur):
    """
    After a match is marked 'completed', update both teams' ratings
    using the simple +25/-25 system and recalculate ladder rankings.

    :param match_id: ID of the completed match.
    :param cur: An open database cursor within the active transaction.
    """
    try:
        ...
        row = cur.fetchone()
        if row is None:
            logger.warning("apply_match_result: match %s not found or not completed", match_id)
            return
        ...
        if winner_id is None:
            logger.warning("apply_match_result: match %s has no winner, skipping rating update", match_id)
            return
        ...
        logger.exception("apply_match_result: error updating ratings for match %s", match_id)
        raise
